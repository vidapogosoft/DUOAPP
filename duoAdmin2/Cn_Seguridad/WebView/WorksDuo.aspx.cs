...
{
    public partial class WorksDuo : System.Web.UI.Page
    {
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            ...
            if (!IsPostBack)
            {
                ...
                img_foto_reg.ImageUrl = string.Format("../Imagenes/Fotos/vidapogosoft/{0}", Path.GetFileName("Works.png"));
                hdfWorksId.Value = "0";
                hdfIdRegistrado.Value = "0";
                hdfIdPerfil.Value = "0";
                hdfRegCodigoUnico.Value = "0";
                hdfFotoRegistrado.Value = "http://18.218.178.167/imagesemail/Works.png";

                CargaRegistradoById(int.Parse(RegId));
                CargaPerfilById(int.Parse(RegId));
                //CargaWorksById(int.Parse(RegId));
            }

        }

        //private void CargaWorksById(int IdRegistrado)
        //{
        //    List<ModeloDatos.Entidades.clsPerfilDUO> res = new List<ModeloDatos.Entidades.clsPerfilDUO>();
        //    res = clsRegistradosBL.ConsultarWorksById(IdRegistrado);

        //    if (res.Count > 0)
        //        DLWorksDuo.DataSource = res;
        //    DLWorksDuo.DataBind();
            ...
            try
            {
                ...
                if (fup_imagen_registrada.HasFile)
                {
                        ...
                        )
                    {
                        ...
                        {
                            fileName = string.Format("{0}{1}", "FotoTrabajo" + DateTime.Now.Ticks.ToString() + "duo" + RegId, Path.GetExtension(fup_imagen_registrada.FileName));
        ...
        protected void RegistrarTrabajo_Click(object sender, EventArgs e)
        {
            int Grabado = 0;

            ModeloDatos.works Perfil = new ModeloDatos.works();

            Perfil.RegWorksId = Convert.ToInt32(hdfWorksId.Value);
            ...
            Perfil.RegDescripcion = TxtAcerca.Text;
            ...
            DisplayModal95();
            Grabado = clsRegistradosBL.SaveTrabajoDuo(Perfil);

            if (Grabado > 0)
            {
                hdfWorksId.Value = "0";
                TxtAcerca.Text = string.Empty;
                img_foto_reg.ImageUrl = string.Format("../Imagenes/Fotos/vidapogosoft/{0}", Path.GetFileName("Works.png"));
                hdfFotoRegistrado.Value = "http://18.218.178.167/imagesemail/Works.png";
